...
urlpatterns = [
    ...
    re_path(r'^user/(?P<pk>[0-9]+)',
            emtct_views.UserUpdate.as_view(), name='user-update'),
    path('users/', emtct_views.UserList.as_view(), name='users-list'),
    re_path(r'^users/(?P<pk>[0-9]+)',
            emtct_views.UserDetail.as_view, name='user-detail'),
    ...
    path('mother_registration/', emtct_views.MotherRegistration.as_view(),
         name='mother_registration'),  # mother_registration
    path('bulk_upload/', emtct_views.BulkUpload.as_view(),
         name='bulk_upload'),
    path('emtct-data-export/', emtct_views.generate_emtct_export,
         name='generate-emtct-export'),
    path('emtct-data-import/', emtct_views.import_ugandaemr_emtct_export,
         name='uganda-emtct-import'),
    path('password-change/',
         PasswordChangeView.as_view(
             template_name='emtct/password_change_form.html'),
         name='password_change'),
    path('password-change/done/',
         PasswordChangeDoneView.as_view(
             template_name='emtct/password_change_done.html'),
         name='password_change_done'),

    path('password-reset/',
         PasswordResetView.as_view(
             template_name='emtct/password_reset_form.html'),
         name='password_reset'),
    path('password-reset/done/',
         PasswordResetDoneView.as_view(
             template_name='emtct/password_reset_done.html'),
         name='password_reset_done'),
    path('reset/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})/',
         PasswordResetConfirmView.as_view(
             template_name='emtct/password_reset_confirm.html'),
         name='password_reset_confirm'),
    path('reset/done/',
         PasswordResetCompleteView.as_view(
             template_name='emtct/password_reset_complete.html'),
         name='password_reset_complete'),
    path('regionjson/', emtct_views.region_list, name='region_list'),
    path('mother_registration/districtjson/<int:id>',
         emtct_views.district_list, name='district_list'),
    path('mother_registration/subcountyjson/<int:id>',
